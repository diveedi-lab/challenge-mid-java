...
public class PageBuilder {
    ...
    public static <T, R> PaginatedResponse<T> build(Selected<R> selected, Function<R, T> mapper, int page) {
        return build(selected, mapper, page, DEFAULT_PAGE_SIZE);
    }

    public static <T,R> PaginatedResponse<T> build(Selected<R> selected, Function<R, T> mapper, int page, int perPage) {
